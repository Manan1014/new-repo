...
app.post("/api/auth/verify", async (req, res) => {
  try {
    ...
    res
      .status(err.response?.status || 500)
      .json(err.response?.data || { error: "Token verification failed" });
  }
});
...
app.post("/api/forecast", authenticateToken, async (req, res) => {
  try {
    ...
    if (invalidRows.length > 0) {
      return res.status(400).json({
        error:
          "Invalid data rows found. Each row must have date, price (number), and quantity (number).",
...
app.get("/api/monthly-data", authenticateToken, async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `${DATA_SERVICE_URL}/monthly-data/${req.user.userId}`,
      {
        params: { year, month },
      }
    );
    ...
    res
      .status(500)
      .json({ error: "Failed to get monthly data", details: err.message });
  }
});

// Get specific month's transactions
app.get(
  "/api/monthly-data/:year/:month/transactions",
  authenticateToken,
  async (req, res) => {
    try {
      const { year, month } = req.params;
      const response = await axios.get(
        `${DATA_SERVICE_URL}/monthly-data/${req.user.userId}/${year}/${month}/transactions`
      );
      res.json(response.data);
    } catch (err) {
      res
        .status(500)
        .json({ error: "Failed to get transactions", details: err.message });
    }
  }
);
...
app.post("/api/monthly-data", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to save monthly data", details: err.message });
  }
});

// Delete monthly data
app.delete(
  "/api/monthly-data/:year/:month",
  authenticateToken,
  async (req, res) => {
    try {
      const { year, month } = req.params;
      const response = await axios.delete(
        `${DATA_SERVICE_URL}/monthly-data/${req.user.userId}/${year}/${month}`
      );
      res.json(response.data);
    } catch (err) {
      res
        .status(500)
        .json({ error: "Failed to delete monthly data", details: err.message });
    }
  }
);
...
app.get("/api/user/profile", authenticateToken, async (req, res) => {
  try {
    const response = await axios.get(
      `${DATA_SERVICE_URL}/user/${req.user.userId}/profile`
    );
    res.json(response.data);
  } catch (err) {
    res
      .status(500)
      .json({ error: "Failed to get user profile", details: err.message });
  }
});
...
app.put("/api/user/profile", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(err.response?.status || 500)
      .json(err.response?.data || { error: "Failed to update user profile" });
  }
});
...
app.put("/api/user/password", authenticateToken, async (req, res) => {
  try {
    const response = await axios.put(
      `${DATA_SERVICE_URL}/user/${req.user.userId}/password`,
      req.body
    );
    res.json(response.data);
  } catch (err) {
    res
      .status(err.response?.status || 500)
      .json(err.response?.data || { error: "Failed to change password" });
  }
});
...
app.get("/api/user/preferences", authenticateToken, async (req, res) => {
  try {
    const response = await axios.get(
      `${DATA_SERVICE_URL}/user/${req.user.userId}/preferences`
    );
    res.json(response.data);
  } catch (err) {
    res
      .status(500)
      .json({ error: "Failed to get user preferences", details: err.message });
  }
});
...
app.delete("/api/user/account", authenticateToken, async (req, res) => {
  try {
    const response = await axios.delete(
      `${DATA_SERVICE_URL}/user/${req.user.userId}/account`,
      {
        data: req.body,
      }
    );
    ...
    res
      .status(err.response?.status || 500)
      .json(err.response?.data || { error: "Failed to delete account" });
  }
});
...
app.get("/api/analytics/summary", authenticateToken, async (req, res) => {
  try {
    const response = await axios.get(
      `${DATA_SERVICE_URL}/analytics/summary/${req.user.userId}`
    );
    res.json(response.data);
  } catch (err) {
    res
      .status(500)
      .json({ error: "Failed to get analytics summary", details: err.message });
  }
});
...
app.get("/api/analytics/trends", authenticateToken, async (req, res) => {
  try {
    const response = await axios.get(
      `${DATA_SERVICE_URL}/analytics/trends/${req.user.userId}`
    );
    res.json(response.data);
  } catch (err) {
    res
      .status(500)
      .json({ error: "Failed to get analytics trends", details: err.message });
  }
});
...
app.get("/api/analytics/categories", authenticateToken, async (req, res) => {
  try {
    const response = await axios.get(
      `${DATA_SERVICE_URL}/analytics/categories/${req.user.userId}`
    );
    res.json(response.data);
  } catch (err) {
    res.status(500).json({
      error: "Failed to get analytics categories",
      details: err.message,
    });
  }
});
...
app.get("/api/analytics/insights", authenticateToken, async (req, res) => {
  try {
    const response = await axios.get(
      `${DATA_SERVICE_URL}/analytics/insights/${req.user.userId}`
    );
    res.json(response.data);
  } catch (err) {
    res.status(500).json({
...
app.get("/api/analytics/report", authenticateToken, async (req, res) => {
  try {
    const response = await axios.get(
      `${DATA_SERVICE_URL}/analytics/report/${req.user.userId}`,
      {
