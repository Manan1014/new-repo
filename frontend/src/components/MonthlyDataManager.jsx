...
const MonthlyDataManager = () => {
  ...
  const fetchTransactions = async (year, month) => {
    try {
      const data = await getMonthTransactions(year, month);
  ...
  const handleDeleteMonthlyData = async (year, month) => {
    if (
      !confirm(
        `Are you sure you want to delete data for ${getMonthName(
          month
        )} ${year}?`
      )
    ) {
      ...
      {monthlyData.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-sm mt-2">
            Upload some sales data to see your monthly summaries here.
          </p>
        </div>
      ) : (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <button
                      onClick={() =>
                        handleDeleteMonthlyData(month.year, month.month)
                      }
            ...
            {selectedMonth ? (
              <div>
                <h3 className="font-semibold text-gray-800 mb-4">
                  Transactions - {getMonthName(selectedMonth.month)}{" "}
                  {selectedMonth.year}
                </h3>
